...
| R4 | 输出规则 | 默认输出到**源文件同目录**（除非用户显式 export 到别处）；支持批量、支持来自不同目录的混合输入；命名为 `.en.srt` / `.zh.srt` 这样的语言后缀 |
...
- 语言后缀：转录 `lecture.en.srt`（英文音频）/ `lecture.zh.srt`（中文音频）；翻译产物同规则。后缀映射表可配置（`en → .en`，`zh → .zh`），新增语言只加一行。
